...
FlutterVulkanImage FlutterGetNextImageCallback(
    ...
    vkAcquireNextImageKHR(g_state.device, g_state.swapchain, UINT64_MAX,
    ...
    return {
        ...
        .format = (unsigned)g_state.surface_format.format,
    };
}
...
void* FlutterGetInstanceProcAddressCallback2(
    void* user_data,
...
TEST_CASE("flutter engine debug mode sample", "[flutter][vulkan]")
{


    std::string project_path = "C:/workspace/ultrawis/build/";
    std::string icudtl_path = "C:/Libraries/flutter/bin/cache/artifacts/engine/windows-x64/icudtl.dat";
    ...
    config.optional_layers.emplace_back(vk::INSTANCE_LAYER_PRE13_SYNCHRONIZATION2);
    auto instance = std::make_shared<vk::Instance>(config);

    g_state.instance = instance->Get();
...

    {
        ...
        for (const auto& pdevice : physical_devices) {
            ...
            vkGetPhysicalDeviceQueueFamilyProperties(pdevice, &qfp_count,
                qfp.data());
...

    {
        ...
        vkCreateFence(g_state.device, &f_info, nullptr,
...

    {
        ...
        config.vulkan.get_instance_proc_address_callback =
            FlutterGetInstanceProcAddressCallback2;
        ...
        std::string assets_path = project_path + "/flutter_assets";
    ...
    vkDestroyCommandPool(g_state.device, g_state.swapchain_command_pool,
        nullptr);
