...
namespace eureka::vulkan
{
    ...
    SwapChain::SwapChain(std::shared_ptr<Window> window,
        ...
        _presentationQueue(presentQueue)
    {
        ...
        auto fitr = std::ranges::find_if(formats, [](const VkSurfaceFormatKHR& sf) {
            return sf.format == VkFormat::VK_FORMAT_B8G8R8A8_UNORM &&
                   sf.colorSpace == VkColorSpaceKHR::VK_COLOR_SPACE_SRGB_NONLINEAR_KHR;
        });

        auto pitr = std::ranges::find_if(
            presentModes, [](const VkPresentModeKHR& pm) { return pm == VkPresentModeKHR::VK_PRESENT_MODE_FIFO_KHR; });
        ...
        _windowResizeConnection = _window->ConnectResizeSlot([this](uint32_t w, uint32_t h) { Resize(w, h); });
    }
    ...
    SwapChainImageReference SwapChain::AcquireNextAvailableImageAsync()
    {
        ...
        if(_outOfData)
        {
            return SwapChainImageReference {.valid = false};
        }

        auto semaphoreHandle = _imageReadySemapores[_currentSemaphore].Get();

        auto [result, imageIndex] =
            _device->AcquireNextSwapChainImage(_swapchain, UINT64_MAX, semaphoreHandle, nullptr);
        ...
        return SwapChainImageReference {.valid = false, .image_index = imageIndex, .image_ready = semaphoreHandle};
    }
    ...
    VkResult SwapChain::PresentLastAcquiredImageAsync(const BinarySemaphoreHandle& renderingDoneSemaphore)
    {
        ...
        VkPresentInfoKHR presentInfo {.sType = VkStructureType::VK_STRUCTURE_TYPE_PRESENT_INFO_KHR,
    ...
    {
        return VkRect2D {{0u, 0u}, {_window->GetWidth(), _window->GetHeight()}};
    }
