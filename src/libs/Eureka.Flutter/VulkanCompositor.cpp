...
namespace eureka::flutter
{
    ...
    void FlutterLayersViewPass::BindToTargetPass(graphics::TargetInheritedData inheritedData)
    {
        ...
        vulkan::PipelineLayoutCreationPreset imguiPipelineLayoutPreset(vulkan::PipelinePresetType::eTexturedRegion,
                                                                       *_globalInheritedData.layout_cache);
        _pipelineLayout = std::make_shared<vulkan::PipelineLayout>(_globalInheritedData.device,
                                                                   imguiPipelineLayoutPreset.GetCreateInfo());
        vulkan::PipelineCreationPreset imguiPipelinePreset(vulkan::PipelinePresetType::eTexturedRegion,
                                                           *_globalInheritedData.shader_cache,
                                                           _pipelineLayout->Get(),
                                                           _targetInheritedData.render_pass->Get());

        _pipeline = vulkan::Pipeline(_globalInheritedData.device,
                                     _pipelineLayout,
                                     _targetInheritedData.render_pass,
                                     imguiPipelinePreset.GetCreateInfo());
    }
    void FlutterLayersViewPass::RecordDraw(const graphics::RecordParameters& params)
    {
        ...
        VkViewport viewport {
            .x = 0.0f, .y = 0.0f, .width = (float)_w, .height = (float)_h, .minDepth = 0.0f, .maxDepth = 1.0f};
        ...
        for(size_t i = 0u; i < _upcomingDrawLayers.size(); ++i)
        {
            ...
            if(pLayer->type == FlutterLayerContentType::kFlutterLayerContentTypeBackingStore)
            {
                ...
                params.command_buffer.Bind(VkPipelineBindPoint::VK_PIPELINE_BIND_POINT_GRAPHICS,
                                           _pipelineLayout->Get(),
                ...
                params.command_buffer.PushConstants(
                    _pipelineLayout->Get(), VkShaderStageFlagBits::VK_SHADER_STAGE_VERTEX_BIT, pushConstanst);
                ...
                params.command_buffer.Draw(6, 1, 0, 0);
            }
        }
    }
            ...
            vulkan::Image2DAllocationPreset::eR8G8B8A8UnormSampledShaderResourceRenderTargetTransferSrcDst))
    {
        ...
        _flutterRendererConfig.vulkan.enabled_device_extension_count =
            _globalInheritedData.device->EnabledExtentions().size();
        _flutterRendererConfig.vulkan.enabled_device_extensions =
            _globalInheritedData.device->EnabledExtentions().data();
        ...
        _backingStoreSampler =
            vulkan::CreateSampler(_globalInheritedData.device, vulkan::SamplerCreationPreset::eLinearClampToEdge);

        _layersViewPass = std::make_shared<FlutterLayersViewPass>(_globalInheritedData);

        _targetPass->AddViewPass(_layersViewPass);

        _lastPresentationTimepoint = CurrentTimeNanoseconds();
    }

    const FlutterRendererConfig& VulkanCompositor::GetFlutterRendererConfig() const
    {
        return _flutterRendererConfig;
    }

    const FlutterCompositor& VulkanCompositor::GetFlutterCompositor() const
    {
        return _flutterCompositor;
    }

    bool VulkanCompositor::CreateBackingStore(const FlutterBackingStoreConfig* config,
                                                     FlutterBackingStore*             backingStoreOut)
    {
        PROFILE_CATEGORIZED_SCOPE(
            "CreateBackingStore", eureka::profiling::Color::Green, eureka::profiling::PROFILING_CATEGORY_RENDERING);
        ...
        std::array<VkDescriptorImageInfo, 1> imageInfo {
            VkDescriptorImageInfo {.sampler = _backingStoreSampler.Get(),
                                   .imageView = bkData->image.GetView(),
                                   .imageLayout = VkImageLayout::VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL}};

        bkData->descriptor_set.SetBindings(0, VkDescriptorType::VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, imageInfo);
    ...
    bool VulkanCompositor::CollectBackingStore(const FlutterBackingStore* /*backingStore*/)
    {
    ...
    void VulkanCompositor::DestroyVulkanBackingStore(BackingStoreData* data)
    {
        PROFILE_CATEGORIZED_SCOPE("DestroyVulkanBackingStore",
                                  eureka::profiling::Color::Green,
                                  eureka::profiling::PROFILING_CATEGORY_RENDERING);
        //DEBUGGER_TRACE("DestroyVulkanBackingStore");
        // TODO, HACKISH to prevent destroting in flight descriptor, maybe pass this to "collect" list that will be released later
        _frameContext->SyncCurrentFrame();
        delete data;
    }

    void VulkanCompositor::EnqueueImageBarriers(const FlutterLayer**              layers,
                                                       size_t                            layersCount,
    ...
    bool VulkanCompositor::PresentLayers(const FlutterLayer** layers, size_t layersCount)
    {
        PROFILE_CATEGORIZED_SCOPE(
            "PresentLayers", eureka::profiling::Color::Green, eureka::profiling::PROFILING_CATEGORY_RENDERING);
    ...
    bool VulkanCompositor::CreateBackingStoreStatic(const FlutterBackingStoreConfig* config,
                                                           FlutterBackingStore*             backingStoreOut,
                                                           void*                            userData)
    {
        return static_cast<VulkanCompositor*>(userData)->CreateBackingStore(config, backingStoreOut);
    }

    bool VulkanCompositor::CollectBackingStoreStatic(const FlutterBackingStore* backingStore, void* userData)
    {
        return static_cast<VulkanCompositor*>(userData)->CollectBackingStore(backingStore);
    }

    void VulkanCompositor::DestroyVulkanBackingStoreStatic(void* userData)
    {
    ...
    bool VulkanCompositor::LayersPresentStatic(const FlutterLayer** layers, size_t layersCount, void* userData)
    {
        return static_cast<VulkanCompositor*>(userData)->PresentLayers(layers, layersCount);
    }

    FlutterVulkanImage VulkanCompositor::GetNextImageStatic(void* /*userData*/,
    ...
    bool VulkanCompositor::PresentImageStatic(void* /*userData*/, const FlutterVulkanImage* /*image*/)
    {
