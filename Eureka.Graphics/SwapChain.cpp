...
namespace eureka
{
   ...
   //     );
		...
		_desc(std::move(desc))
    {
        ...
        auto deviceName = deviceContext.PhysicalDevice()->getProperties().deviceName;
        std::string deviceNameStr(deviceName.begin(), deviceName.end());

        if (deviceNameStr.contains("Intel") && deviceNameStr.contains("UHD"))
        {
            DEBUGGER_TRACE("GPU Identified as Intel UHD, setting presentation mode to immediate - possible driver bug");
            _selectedPresentMode = vk::PresentModeKHR::eImmediate;
        }
